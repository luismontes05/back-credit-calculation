...
def get_client(request: Request,id: int=0,  num_document: str=''):
    data_client = ClientService(request).get_client(id, num_document)
...
def get_reference(request: Request,id: int=0,  client_id: int=0):
    data_reference = ClientService(request).get_reference(id, client_id)
...
def get_propertie(request: Request,id: int=0,  client_id: int=0):
    data_propertie = ClientService(request).get_propertie(id, client_id)
...
def create_client(request: Request, new_client: ClientShema) -> dict:
    new_client = ClientService(request).create_client(new_client)
...
def create_reference(request: Request, new_reference: Referencesshema) -> Referencesshema:
    new_reference = ClientService(request).create_reference(new_reference)
...
def create_propertie(request: Request, new_propertie: PropertiesShecma) -> PropertiesShecma:
    new_propertie = ClientService(request).create_propertie(new_propertie)
...
def update_client(id: int, request: Request, update_client: ClientShemaUpdate) -> dict:
    result = ClientService(request).update_client(id, update_client)
...
def update_reference(id: int, request: Request, update_reference: Referencesshema) -> dict:
    result = ClientService(request).update_reference(id, update_reference)
    return JSONResponse(status_code=200, content=result)
...
def update_propertie(id: int, request: Request, update_propertie: PropertiesShecma) -> dict:
    result = ClientService(request).update_propertie(id, update_propertie)
    return JSONResponse(status_code=200, content=result)
...
def delete_reference(id: int, request: Request) -> dict:
    result = ClientService(request).delete_reference(id)
    return JSONResponse(status_code=200, content=result)
...
def delete_propertie(id: int, request: Request) -> dict:
    result = ClientService(request).delete_propertie(id)
    return JSONResponse(status_code=200, content=result)

@client_router.get('/api/client/type_properties', tags=['client'], response_model=dict, status_code=200, dependencies=[Depends(JWTBearer())])
def get_client(request: Request):
    data = ClientService(request).type_properties()
